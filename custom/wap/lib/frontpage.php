...
class wap_frontpage extends wap_controller{
    ...
    function __construct(&$app){
        ...
        if(!stripos($_SERVER['REQUEST_URI'],'adsfssfsfs') && !isset($_SESSION['account']['member'])){
            $this->set_weixin_login();
        }
        // if(!stripos($_SERVER['REQUEST_URI'],'jlsfsfssfsf') && !isset($_SESSION['account']['pay_attention_to'])){
        //     $this->set_weixin_attention();
        // }
        if(isset($_SESSION['pay_attention_to']) && !stripos($_SERVER['REQUEST_URI'],'attention')){
            $this->pagedata['pay_attention_to']=$_SESSION['pay_attention_to'];
        }
        
    }
    public function set_weixin_login(){
         // 判断是否微信
        ...
        if (strpos($user_agent, 'MicroMessenger') === false) {
            ...
            if(!isset($_SESSION['account']['member'])){
                ...
                $this->redirect('https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$weixinbind['appid'].'&redirect_uri='.urlencode(WWW_URL.'/index.php/wap/gallery-adsfssfsfs.html').'&response_type=code&scope=snsapi_base&state='.WWW_URL.$_SERVER['REQUEST_URI'].'#wechat_redirect');
            }
        }
    }
