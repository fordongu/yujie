...
class microshop_finder_shop {
    ...
    public $column_shop_account = '会员账户';
    function column_shop_account($row) {
        $mdl    = app::get('pam')->model('members');
        $filter=array();
        $filter['member_id']=$row['member_id'];
        //$filter['login_type']='local';
        $info   = $mdl->dump($filter);
        $account = $info['login_account'];
