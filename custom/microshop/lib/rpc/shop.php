        ...
        if ( $shop_id > 0 || $member_id > 0 ) {
            ...
            if ( $info ) {
                if ( $info['is_open'] == 'on' ) {
                    ...
                    if($shop_id==0){
                        $data=$member->dump(array('member_id'=>$member_id),'avatar');
                    }else{
                        $data=$member->dump(array('member_id'=>$shop_id),'avatar');
                    }
                    if(empty($data['avatar'])){
                        $avatar=base_storager::image_path(app::get('b2c')->getConf('site.avatar'), 's');
                    }else{
                        $avatar=base_storager::image_path($data['avatar'], 's');
                    }
                    ...
                    $ret    = array (
                        ...
                        'data'  => $info,
                        'avatar'=>$avatar,
                        'shop_link'=>WWW_URL.'index.php/microshop-'.$member_id.'.html',
