...
class microshop_mdl_special extends dbeav_model {
    ...
    function getSpecialInfo( $param = array(), $update_visit = 1 ) {
        ...
        if ( $param['special_id'] ) {
            ...
            $imagegoods=$g_mdl->getList('*', array('goods_id' => $info['goods_id']));
            $info['goods']=$imagegoods[0];
            $info['avatar']=base_storager::image_path($imagegoods[0]['image_default_id'], '');
            ...
            if ( $list ) {
                ...
                if ( $p_ids ) {
				    ...
				    $val['cost'] = floor($val['cost']);
				    //$val['mktprice'] = floor($val['mktprice']);
				    $val['commission'] = floor($val['commission']);
                                    $list[$k]['detail']   = $val;
                                }
                            }
                            $v['platform']          = $platform;
     ...
     * @return  array
     */
    function getDetail($special_id = 0,$field='*') {
        if(empty($special_id)){
            return NULL;
        }
