...
class mobileapi_rpc_order extends mobileapi_frontpage {
    ...
    public function __construct(&$app) {
        ...
        $this->verify_member();
        $this->Orders = $this->app->model('orders');
...
//         print_r($arr_args);exit;
        ...
        if (isset($aCart['cart_status'])) {
            ...
            $this->Rpc->send_user_error('cart_error', $aCart['cart_error_html']);
        }
        ...
        if ($this->mCart->is_empty($aCart)) {
            ...
            $this->Rpc->send_user_error('cart_error', '操作失败，购物车为空！');
        }
        // 校验购物车有没有发生变化
...
//         	$this->end(false,app::get('b2c')->_('购物车内容发生变化，请重新结算！'),$this->gen_url(array('app'=>'b2c','ctl'=>'site_cart','act'=>'checkout')),true,true);
            $this->Rpc->send_user_error('cart_change_error', '购物车内容发生变化，请重新结算！');
        }
...
//             $this->end(false, $msg,null,true,true);
            $this->Rpc->send_user_error('order_error', $msg);
        } else {
...
//                 $this->end(false,app::get('b2c')->_('收货地址不完整，请填写邮编'),null,true,true);
                $this->Rpc->send_user_error('order_error', '收货地址不完整，请填写邮编');
            }
...
//                 $this->end(false, $this->app->_("ship_method_consistent_error"),'',true,true);
                $this->Rpc->send_user_error('order_error', 'ship_method_consistent_error');
            }
        }
...
//                 $this->end(false, $message);
                $this->Rpc->send_user_error('order_error', $message);
        }
...
//所属仓库门店
    ...
    function payment_change() {
        ...
        if ($pay['pay_app_id'] == -1) {
//     		return true;
            $this->Rpc->send_user_error('pay_app_id_error', '支付ID错误!');
        }
        ...
        $this->Rpc->send_user_error('4003', '更新支付方式失败');
    }
    ...
    public function confirm($order_id) {
        if ($order_id) {
            ...
            $db = kernel::database();
            $transaction_status = $db->beginTransaction();
            $filter = array('order_id' => $order_id['order_id'], 'member_id' => $this->app->member_id);
            $data = array('status' => 'finish', 'confirm' => 'Y');
            $datas = array('status' => 'succ', 't_confirm' => time());
            if ($this->Orders->update($data, $filter) && $this->app->model('reship')->update($datas, $filter)) {
                $db->commit($transaction_status);
                return '修改成功';
            } else {
                $db->rollBack();
    ...
    public function return_orders() {
        ...
        $data = $objOrder->getList('name,price,nums,product_id', array('order_id' => $order_id));
        if (!empty($data)) {
    ...
    //点击订单查询会员信息
    public function commission_members($order_id){
        if($order_id){
            $log=$this->app->model('order_items');
            $order=$log->dump(array('order_id'=>$order_id['order_id']),'nums,name,price');
            return $order;
        }else{
            $msg = '参数错误！';
        }
