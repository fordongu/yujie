...
class mobileapi_rpc_comment extends mobileapi_frontpage{
    ...
    private function _check_vcode($item,$_POST){
        ...
        if(!$flag){
            //$this->Rpc->send_user_error('4003', app::get('b2c')->_('验证码填写错误'));
        }
        return $flag;
    ...
    */
    ...
    private function _check_post($_POST,$type){
        ...
        //验证基本参数
        if(!$_POST['goods_id']){
        	 $this->Rpc->send_user_error('4003', app::get('b2c')->_('参数错误'));
        }else{
        ...
        if(empty($_POST['comment'])){
        	$this->Rpc->send_user_error('4003', app::get('b2c')->_('内容不能为空'));
        }
        ...
        if(!$disask->toValidate($type,$_POST,$message) ){
            if($message){
                ...
                $this->Rpc->send_user_error('4003', app::get('b2c')->_('权限不足'));
            }
        }
        //验证码验证
    ...
    public function toComment($param = array()){
        ...
        if($comment_id = $objComment->send($aData, $item, $message)){
            ...
            $this->Rpc->send_user_error('4003', app::get('b2c')->_('发表失败'));
        }
    }
}
