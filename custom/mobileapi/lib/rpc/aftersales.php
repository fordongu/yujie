...
class mobileapi_rpc_aftersales extends mobileapi_rpc_member
{
    /**
     * 构造方法
     ...
     *退换货记录
     * */
    public function afterrec($nPage)
    {
        if($nPage){
            $nPage=$nPage['page_no'];
        }else{
        ...
        $aData=$product->getList('*',$filter,($nPage-1)*10,10);
        return $aData;
    }
    ...
    public function add($params = array()){
        ...
        $objOrder = $this->app->model('order_items');
        $goods = $this->app->model('goods');
        $data=$objOrder->getList('name,price,nums,product_id,goods_id',array('order_id'=>$order_id));
        if(!empty($data)){
            foreach($data as $key=>$vo){
                $good=$goods->dump(array('goods_id'=>$vo['goods_id']),'bn,image_default_id');
                $data[$key]['bn']=$good['bn'];
                if(empty($good['image_default_id'])){
                    $avatar=app::get('image')->getConf('image.set');
                    $data[$key]['avatar']=base_storager::image_path($avatar['S']['default_image'], 's');
                }else{
                    $data[$key]['avatar']=base_storager::image_path($good['image_default_id'], 's');
                }
            }
        }
        $arr=array();
    ...
    public function return_save()
    {
        ...
        if (!isset($obj_return_policy) || !is_object($obj_return_policy))
        {
            $this->Rpc->send_user_error('error', '售后服务应用不存在！');

        }

        if (!$obj_return_policy->get_conf_data($arr_settings))
        {
            $this->Rpc->send_user_error('error', '售后服务信息没有取到！');
        }

        if (!$_POST['product_bn'])
        {
            $this->Rpc->send_user_error('error', '您没有选择商品，请先选择商品！');
        }

        if (!$_POST['title'])
        {
            $this->Rpc->send_user_error('error', '请填写退货理由');
        }

        $upload_file = "";
        if (isset($_FILES) && $_FILES['file']['size'] > 314572800 )
        {
            $this->Rpc->send_user_error('error', '上传文件不能超过300M！');
        }
        ...
        if (isset($_FILES) &&  $_FILES['file']['name'] != "" )
        {
            ...
            {
                $text = implode(",", $type);
                $this->Rpc->send_user_error('error',app::get('aftersales')->_("您只能上传以下类型文件: ") . $text . "<br>");
            }
        ...
        if(!$_POST['agree']){
            $this->Rpc->send_user_error('error',app::get('aftersales')->_("请先查看售后服务须知并且同意"));
        }
        ...
        $obj_aftersales = kernel::single('b2c_api_rpc_notify_aftersales');
        ...
        {
            $this->Rpc->send_user_error('error', $msg);
        }
    }


    //wap端退款数据提交处理
    public function return_wap_save()
    {
        $obj_return_policy = kernel::service("aftersales.return_policy");
        $arr_settings = array();
        ...
        if (!isset($obj_return_policy) || !is_object($obj_return_policy))
        {
            return array('error'=>'售后服务应用不存在！');

        }

        if (!$obj_return_policy->get_conf_data($arr_settings))
        {
            return array('error'=>'售后服务信息没有取到！');
        }

        if (!$_POST['product_bn'])
        {
            return array('error'=>'您没有选择商品，请先选择商品！');
        }

        if (!$_POST['title'])
        {
            return array('error'=>'请填写退货理由');
        }
