...
class mobileapi_rpc_cart extends mobileapi_frontpage {
    ...
    public function batch_add_cart($params = array(), $rpcService) {
        ...
        foreach ($goodsObj as $key => $value) {
            ...
            if ($value->buy_code)
                $_REQUEST['buy_code'] = $value->buy_code;
            if ($value->btype)
                $_REQUEST['btype'] = $value->btype;
            $this->add();
            $_REQUEST = null;
        }
    ...
    public function add() {
        ...
        /*
          if(!$product_info){
          $this->Rpc->send_user_error('need_login', '产品ID错误!');
          }
         */
        ...
        if ($num > $product_info['store'] - $product_info['freez']) {
            $this->Rpc->send_user_error('store_error', '产品库存不够本次购买');
        }

        if ($buy_code)
            $goods['extends_params'] = array('buy_code' => "$buy_code");
        ...
        $this->Rpc->send_user_error('cart_add_error', $re['info']);
    }
    ...
    public function add_coupon() {
        ...
        if ($return_coupon_data) {
            ...
            $this->Rpc->send_user_error('coupon_error', '优惠券添加使用失败!');
    ...
    public function get_list() {
        $this->_common(1);
        $this->pagedata['aCart']['subtotal_prefilter'] = $this->objMath->number_minus(array($this->pagedata['aCart']['subtotal'], 0));
        $this->pagedata['aCart']['promotion_subtotal'] = $this->objMath->number_minus(array($this->pagedata['aCart']['subtotal'], 0));
    ...
    public function update() {
        ...
        if (!$_flag) {
            $this->Rpc->send_user_error('cart_update_error', '购物车修改失败！');
        }
    ...
    public function remove() {
        ...
        if (!$flag) {
            $this->Rpc->send_user_error('cart_remove_error', $msg);
        }
    ...
    public function checkout() {
        ...
        $total_item = $this->objMath->number_minus(array($this->pagedata['aCart']["subtotal"], 0));
        // 取到商店积分规则
        ...
        $total_amount = $this->objMath->number_minus(array($this->pagedata['aCart']["subtotal"], 0));
        if ($total_amount < 0)
            $total_amount = 0;
    ...
    private function _common($flag = 0, $is_fastbuy = false) {
        ...
        if ($this->show_gotocart_button)
            $this->pagedata['show_gotocart_button'] = 'true';
        ...
        if ($this->ajax_update === true) {
            foreach (kernel::servicelist('b2c_cart_object_apps') as $object) {
                ...
                //应该判断是否实现了接口
                if (!method_exists($object, 'get_update_num'))
                    continue;
                ...
                if ($this->pagedata['edit_ajax_data']) {
                    ...
                    if ($object->get_type() == 'goods') {
                        ...
                        if (!method_exists($object, 'get_error_html'))
                            continue;
    ...
    private function _item_to_disabled(&$aCart, $flag) {
        ...
        foreach ((array) $aCart['object'] as $_obj_type => $_arr_by_obj_type) {
            ...
            if (isset($arr_cart_disabled_session[$_obj_type])) {
                if (!$o[$_obj_type])
                    continue;
                if (!method_exists($o[$_obj_type], 'apply_to_disabled'))
                    continue;
