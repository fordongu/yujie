...
class ectools_regions_operation
{
     ...
     * 同步地区地址
     */
    public function replaceDlArea($aData,&$msg) {
        $is_save = false;
        ...
        $db     = kernel::database();
        $sql    = 'REPLACE INTO '.$db->prefix.'ectools_regions ('.implode(',', $key).') VALUES ('.implode(',', $val).');';
        $db->exec($sql);
        $cPath = $this->model->dump(intval($aData['region_id']), 'region_path');
        if ( $this->updateSubPath($cPath['region_path'],$aData['region_path']) ) {
            return $this->updateRegionData();
        }
        return false;
    }
