...
class ectools_pay extends ectools_operation {
    ...
    public function generate(&$sdf, &$controller = null, &$msg = '') {
        ...
        $pay_app_type = empty($sdf['pay_app_type']) ? '' : $sdf['pay_app_type'];
        unset($sdf['pay_app_type']);
        // 异常处理
        ...
        if ($sdf['pay_type'] == 'online') {
            ...
            if ($sdf['pay_app_id'] == 'wxpay' && $pay_app_type == 'mobile') {
                //logger::info("APP微信支付请求信息: " . var_export($sdf, 1));
                return 'mwxpay';
            } elseif ($sdf['pay_app_id'] == 'wxpay') {
                $user_agent = $_SERVER['HTTP_USER_AGENT'];
                if (strpos($user_agent, 'MicroMessenger') === false) {
                    // 非微信浏览器禁止浏览
                    logger::info('微信支付不能使用非微信浏览器支付,浏览器版本号: ' . var_export($user_agent, 1));
                } else {
                    //logger::info("wap微信支付请求信息：" . var_export($sdf, 1));
                    $is_payed = kernel::single('weixin_payment_pay4_wxpayjsapi')->dopay($sdf);
                }
            } elseif ($pay_app_id != 'deposit') {
