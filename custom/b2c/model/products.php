...
class b2c_mdl_products extends dbeav_model{
    ...
    function getOne($product_id){
        ...
        $sql = 'SELECT p.goods_id, p.product_id, p.name, p.price, p.mktprice, p.title, g.image_default_id FROM sdb_b2c_products p,sdb_b2c_goods g WHERE p.goods_id = g.goods_id and p.marketable = "true" and p.product_id = '.$product_id;
