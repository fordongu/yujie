...
//            return $this->ioTitle['csv'][$filter['type_id']];
    ...
    function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null,&$total=false){
        /* $key = md5(json_encode($filter).json_encode($cols).json_encode($offset).json_encode($orderType));
        ...
        if($searchrule && $filter['search'] === true){
            unset($filter['search']);
            $data = $searchrule->getList($cols,$filter,$offset,$limit,$orderType,$total);
        }else{
            unset($filter['search']);
            $total=false;
    ...
    function wFilter($words){
        ...
        foreach($word as $k=>$v){
            if($v){
                ...
                $sql[]='(name LIKE \'%'.$word[$k].'%\' or bn like \''.$word[$k].'%\' or barcode like \''.$word[$k].'%\' '.( $goodsId?' or goods_id IN ('.implode(',',$goodsId).') ':'' ).')';
            }
        }
...
//                  }
    ...
    //根据goods_id 获取tc_id
    public function get_tc_id($goods_id){
        $res = $this->getList('tc_id',array('goods_id'=>$goods_id));
        if($res[0]['tc_id'] > 0){
            return $res[0]['tc_id'];
        }
        logger::info('goods_id : '.$goods_id.' tc_id is empty;');
        return false;
    }
}
