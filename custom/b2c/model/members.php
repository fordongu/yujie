...
class b2c_mdl_members extends dbeav_model{
    ...
    function save(&$sdf,$mustUpdate = null, $mustInsert=false){
        ...
        if(!empty($sdf['nickname']) && $sdf['member_id'] > 0){
            kernel::single('b2c_huanxin_registhuanxin')->edit_nickname($sdf['member_id'],$_sdf['nickname']);
        }
    ...
    public function get_member_type($member_id){
        if($member_id > 0){
            $res = $this->getList('member_lv_id',array('member_id'=>$member_id));
            $res = kernel::single('b2c_mdl_member_lv')->getList('role,class',array('member_lv_id'=>$res[0]['member_lv_id']));
            $res[0]['member_id'] = $member_id;
            switch ($res[0]['class']) {
                case 'one':
                    $res[0]['num'] = 1;
                    break;
     ...
     * 
     */
    public function add_commission($member_id,$commission) {
        $filter = array('member_id'=>$member_id);
        $o_co = $this->dump($filter,'commission,advance_freeze,advance');
        if($commission > 0){
           $data = array(
            'commission'=>($o_co['commission'] + $commission),
            'advance_freeze'=>($o_co['advance_freeze'] + $commission),
            );
           return $this->update($data,$filter); 
        }else{
            $data = array(
            'advance'=>($o_co['advance'] - $commission),
            'advance_freeze'=>($o_co['advance_freeze'] + $commission),
            );
           return $this->update($data,$filter); 
        }
