    ...
    public function check_login($mini) {
        if ($this->userObject->is_login()) {
            ...
            if ($_GET['mini_passport'] == 1 || $mini) {
                kernel::single('site_router')->http_status(302);
                return;
            } else {
                //您已经是登录状态，不需要重新登录
                $this->redirect($url);
...
//end function
    ...
    public function login_ajax_account() {
        ...
        if ($this->userPassport->check_login_account($login_account, $msg)) {
            echo json_encode(array('needVerify' => 'true'));
            exit;
        } else {
            echo json_encode(array('needVerify' => 'false'));
            exit;
        }
    }
    ...
    public function post_login() {
        ...
        if (!$member_id) {
            ...
            $this->splash('failed', null, $msg, true, $data);
            exit;
        }
...
//end function
    ...
    public function signup($url = null) {
        // header("Location:".WWW_URL."index.php/wap/passport-signup.html");exit();
        preg_match('/passport-signup-(\d+).html/', $_SERVER['REQUEST_URI'], $arr);
        $this->pagedata['inivte_id'] = $arr[1];
        ...
        $this->set_tmpl('passport');
        //注册是否需要验证码
        $this->pagedata['valideCode'] = false;
    ...
    public function signup_ajax_check_name() {
        ...
        if ($flag) {
            ...
            $this->splash('success', null, $this->app->_('该登录账号可用'), true);
            exit;
        } else {
    ...
    public function create() {
        ...
        if( !$this->userPassport->check_signup($_POST,$msg) ){//短信验证码检查

             $this->splash('failed',null,$msg,true);exit;
        }
        //被邀请的用户注册处理
        kernel::single('base_session')->start(); //开启session
        if ($_SESSION['inivte_id'] > 0) {
            $invite_mem_fid = $_SESSION['inivte_id'];
        } elseif (!empty($_POST['invitation_code'])) {
            $res = kernel::single('pam_mdl_members')->getList('member_id', array('login_account' => $_POST['invitation_code']));
            if ($res[0]['member_id'] > 0) {
                $invite_mem_fid = $res[0]['member_id'];
            } else {
                $this->splash('failed', null, '您填写的邀请码不对', true);
                exit();
            }
        }else{
            $invite_mem_fid = kernel::single('b2c_ticheng_rand')->invitecode();
        }
        

        $saveData = $this->userPassport->pre_signup_process($_POST);
        $saveData['b2c_members']['nickname'] = $_POST['pam_account']['nickname'];
        $saveData['b2c_members']['mobile'] = $_POST['pam_account']['login_account'];
        $flag = $this->userPassport->check_signup_account(trim($_POST['pam_account']['login_name']), $msg);
        if (!$flag) {
            $this->splash('failed', null, app::get('b2c')->_('手机号已被注册'), true);
            exit();
        }
    ...
    public function send_vcode_sms() {
        ...
        logger::info('admin0778eee9:'.  var_export($mobile,1));
        $login_type = $this->userPassport->get_login_account_type($mobile);
        if ($login_type != 'mobile') {
            $msg = app::get('b2c')->_('请填写正确的手机号码');
            $this->splash('failed', null, $msg, true);
        }
        logger::info('admin0778eee9:'.  var_export($login_type,1));
        ...
        logger::info('admin0778eee9:'.  var_export($vcode,1));
        if ($vcode) {
            //发送验证码 发送短信
            $data['vcode'] = $vcode;
            logger::info('admin0778eee9:'.  var_export($data,1));
