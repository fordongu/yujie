...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    function ajax_product_price($product_id){
        ...
        if(!cachemgr::get('ajax_product_price'.$product_id,$price)){
            ...
            $price = $this->_get_product_price($product[0]['product_id'],$aGoods,$this->arr_member_info['member_lv']);
            if($price['mlv_price']){
    ...
    function _get_product_price($productId,$aGoods,$member_lv){
        ...
        if($member_lv){
        	$sSql = "SELECT p.product_id,p.price
                ...
                WHERE p.level_id='$member_lv' AND p.product_id = '$productId' ";
        	$db     = kernel::database();
        	$aPrice = $db->select($sSql);
        	
        	if(isset($aPrice[0]['price'])){
        		$productMemberPrice['price'] = $aPrice[0]['price'];
        	}
        }
    ...
    */
	...
	//加载erp订时器 请不要删除否则订单无法同步
	 public function erp_ordersrtar() {
		 $ectoolsPay = kernel::single('ectools_pay');
		$ectoolsPay->erpskuinfo();
		file_put_contents("erplog.txt", date("Y-m-d H:i",time())."\r\n", FILE_APPEND);
