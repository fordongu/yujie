...
class b2c_ctl_site_member extends b2c_frontpage{
    ...
    private function get_cpmenu(){
        ...
        $info   = $member_model->dump(array('member_id'=>$this->app->member_id));
        if ($info['member_type'] == 1) {
            $arr_microshop =  array('label'=>app::get('b2c')->_('我要开店'),'app'=>'b2c','ctl'=>'site_member','link'=>'open_shop');
        } else {
        ...
        $arr_bases = array(
            ...
            array(
                ...
                'items'=>array(
                 ...
                 //   array('label'=>app::get('b2c')->_('我的代金券'),'app'=>'b2c','ctl'=>'site_member','link'=>'voucher'),
                    $arr_blance,//预存款充值
                    $arr_recharge_blance,//预存款充值
                    array('label'=>app::get('b2c')->_('我要提现'),'app'=>'b2c','ctl'=>'site_member','link'=>'withdrawal'),
                    array('label'=>app::get('b2c')->_('我的银行账户'),'app'=>'b2c','ctl'=>'site_member','link'=>'myBank'),
                    $arr_point_coupon_exchange,//积分兑换优惠券
                    array('label'=>app::get('b2c')->_('我的合伙人'),'app'=>'b2c','ctl'=>'site_member','link'=>'partner'),
                ),
            ),
            array(
            ...
            array(
                'label'=>app::get('b2c')->_('移动财富'),
                'mid'=>6,
                'items'=>array(
                    array('label'=>app::get('b2c')->_('二维码邀请'),'app'=>'b2c','ctl'=>'site_member','link'=>'erCode'),
                  //  array('label'=>app::get('b2c')->_('商品分享'),'app'=>'b2c','ctl'=>'site_member','link'=>'shareGoods'),
    ...
    public function erCode() {
        ...
        $this->pagedata['value'] = WWW_URL.'index.php/passport-signup-'.$member_id.'.html';;
        $this->output();
    }
    ...
    public function partner(){
        ...
        $aData = $data->direct_referrals(array('member_id'=>$this->member['member_id']));
        $this->pagedata['title']=app::get('b2c')->_('我的合伙人');
      
        $this->pagedata['browser'] = $this->get_browser();
        $this->pagedata['partner'] = $aData;
        $this->output();
    ...
    public function partner_list($member_id){
        $data=kernel::single('mobileapi_rpc_passport');
        $aData=$data->direct_referrals(array('member_id'=>$member_id));
        $this->pagedata['title']=app::get('b2c')->_('合伙人');
        
        $this->pagedata['controller'] = "partner";
        $this->pagedata['browser'] = $this->get_browser();
        $this->pagedata['partner'] = $aData;
        $this->output();
    }
    ...
    public function withdrawal($param = array(), $rpcService) {
        ...
        //提现次数
        $rs = $db->select("SELECT count(*) as total FROM `sdb_b2c_member_withdrawal` WHERE member_id='" . $this->app->member_id . "'");
        if ($rs[0]['total']) {
            $total_results = $rs[0]['total'];
        }
    ...
    public function submit_withdrawal() {
        ...
        //验证提现账户
        if(empty($bank)){
            $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'showBank'));
            echo '<meta http-equiv="refresh" content="2;'.$url.'">';
            $this->splash('success',$url,app::get('b2c')->_('您还没有绑定提现的账户,请先绑定!'));
        }
        
        $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'withdrawal'));
        echo '<meta http-equiv="refresh" content="2;'.$url.'">';
        
        // 验证
        if (!preg_match('/^\d*$/', $money) || $money <= 0 || ($money % 100) != 0) {
            $this->splash('success',$url,app::get('b2c')->_('提交的金额不是数字或者金额小于0了！以佰元为单位！'));
        }
        ...
        if (($total['advance']['total'] - $money) < 0) {
            $this->splash('success',$url,app::get('b2c')->_('您当前的预存款余额不足'));
        }
        ...
        if ($mem_wit->insert($arr)) {
            ...
            $this->splash('success',$url,app::get('b2c')->_('申请错误稍后再试'));
        }
    }
     ...
     * 
     */
    public function showBank($bank_id) {
        
        //我的账号
        $MemberBank = app::get('b2c')->model('member_bank');
        $my_bank = $MemberBank->dump($bank_id);
        $this->pagedata['my_bank'] = $my_bank;
        ...
        $this->pagedata['browser'] = $this->get_browser();
        $this->pagedata['bank_list'] = $bank_list;
        $this->pagedata['controller'] = "myBank";
    ...
    public function submitBank(){
        ...
        $account_type = $userPassport->get_login_account_type($account);
        
        //验证账号是否正确
        $is_legal = FALSE;
        
        if(strpos($bank_name, '支付宝') && in_array($account_type, array('email','mobile'))){
            $is_legal = TRUE;
        }elseif(strpos($bank_name, '浦发') && is_numeric($account) && strlen($account) == 16){
            $is_legal = TRUE;
        }elseif(is_numeric($account) && strlen($account) == 19){
            $is_legal = TRUE;
        }
   
        if(!$is_legal){
           
            $this->splash('success',$url,app::get('b2c')->_('您绑定的卡号不正确,请重新输入!'));
        }
      
        //插入数据
        $MemberBank = app::get('b2c')->model('member_bank');
        $arr['member_id'] = $this->app->member_id;
        $arr['bank_num'] = $account;
        $arr['create_time'] = time();
        $arr['bank_name'] = $bank_name;
        $arr['real_name'] = $name;

        if ($MemberBank->insert($arr)) {
            $message = app::get('b2c')->_('添加成功!');
           
        } else {
            $message = app::get('b2c')->_('添加失败!');
        }
    ...
    public function delBank($bank_id){
        $MemberBank = app::get('b2c')->model('member_bank');
        $result = $MemberBank->delete(array('member_bank_id'=>$bank_id));
        
        $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'myBank'));
        echo '<meta http-equiv="refresh" content="2;'.$url.'">';
        
        if ($result) {
            $message = app::get('b2c')->_('删除成功!');
           
        } else {
            $message = app::get('b2c')->_('删除失败!');
        }
        $this->splash('success',$url,$message);
    }
    ...
    function setting(){
        ...
        $userObject = $this->app->model('members');
        $membersData = $userObject->dump(array('member_id'=>$this->app->member_id),'*');
    ...
    function save_setting(){
       ...
       $data=kernel::single('mobileapi_rpc_member');
       $member=$data->edit_nickname();
        if($member){
    ...
    */
     ...
     * 我的代金券
     * */
    public function voucher($page=1){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('我的代金券'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $vouModel = app::get('b2c')->model('voucher');
        $aData = $vouModel->getList('*',array('member_id'=>$this->app->member_id));
        if($aData){
            foreach ($aData as $k => &$item) {

                $curTime = time();
                if ($curTime>=$item['create_time'] && $curTime<$item['vou_to_time']) {
                    if($item['vou_isvalid']!=false){
                        $item['vou_status'] = app::get('b2c')->_('可使用');
                    }else{
                        $item['vou_status'] = app::get('b2c')->_('本代金券已作废');
                    }

                    if($item['disabled']==false){
                        $item['vou_status'] = app::get('b2c')->_('本代金券次数已用完');
                    }
                }else{
                    $item['vou_status'] = app::get('b2c')->_('还未开始或已过期');
                }
            }
        }
        $this->pagination($nPage,ceil(count($total)/$this->pagesize),'coupon');
        $this->pagedata['vouchers'] = $aData;
        $this->output();
    }
    ...
    public function securitycenter(){
        ...
        $datas = app::get('b2c')->model('member_auth')->getList('*',array('member_id'=>$member_id));
        $array = app::get('b2c')->model('member_auth')->getList('*',array('member_id'=>$member_id));
        foreach($datas as $row) {
            if ($row['is_checked'] == 2) {
                $data['sfz'] = $row['identity_num'];
                $verify['sfz'] = true;
            }elseif($row['is_checked'] == 0){
                $data['sfz'] = "审核中...";
                $verify['sfz'] = true;
            }
        }
...
//    }
    ...
    function verify_vcode2($verifyType){
      if($_POST['send_type'] == 'sfz'){
          //error_log(var_export($_FILES,1), 3, DATA_DIR."/err.txt");
          if(!empty($_POST['real_name'])){
              if(strlen($_POST['identity_num']) == 18){
                  $arr['member_id'] = $this->app->member_id;
                  $arr['real_name'] = $_POST['real_name'];
                  $arr['identity_num'] = $_POST['identity_num'];
                  //$arr['upimage'] = PA.$image;
                  //$arr['image_id'] = $image['image_id'];
                  $arr['uptime'] = time();
                  app::get('b2c')->model('member_auth')->insert($arr);
              }else{
                  $msg = app::get('b2c')->_('身份证号码不合法');
                  $this->splash('failed',null,$msg,true);exit;
              }
          }else{
              $msg = app::get('b2c')->_('真实姓名不能为空');
              $this->splash('failed',null,$msg,true);exit;
          }
      }
      ...
      $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'verify2_1','arg0'=>$verifyType));
      $this->splash('success',$url);
    }
    function verify_vcode2_1($verifyType){
        unset($_SESSION['vcodeVerifykey']['activation']);
        $up = new upload(ROOT_DIR."/public/sfz");
        for($i=1;$i<=2;$i++){
            $arr[] = PA.$up->move();
        }
        $image = implode('|', $arr);
        ...
        $data = app::get('b2c')->model('member_auth')->update(array('upimage'=>$arr['upimage']),$filter);
        if($data){
              // $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'verify3','arg0'=>$verifyType));
              // $this->splash('success',$url);
            //echo "上传成功";
            $this->page('site/member/verify3.html');
        }
    }   
    ...
    {
        if (!trim($_POST['agency_id']) && !trim($_POST['shop_name'])) {
            $this->splash('failed',null,'门店编码与微店名称不能为空',true);
        }

        $mdl_microshop = app::get('microshop')->model('shop');
        $info = $mdl_microshop->dump(array('member_id', $this->app->member_id));
        if ($info) {
            $this->begin(array('app' => 'b2c','ctl' => 'site_member', 'act'=>'index'));
            $this->end(false, app::get('b2c')->_('您已经开过微店了！'));
        }

        $_filter    = array (
            'agency_no' => trim($_POST['agency_id']), //更新为经销商编号
            'member_type'   => 3,
        );
        $mdl_members  = $this->app->model('members');
        $_row = $mdl_members->dump($_filter);
        if ( isset($_row['member_id']) && $_row['member_id'] > 0 ) {
            ...
            if ($mdl_microshop->save($data) ) {
                // 修改用户状态
                $mdl_members->update(array('member_type' => 2), array('member_id' => $this->app->member_id));
                $this->splash('success',$this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'promotion_into_logs')),app::get('b2c')->_('开店成功'),true);
            } else {
                $this->splash('failed',null,'系统错误,请联系管理员',true);
            }
        } else {
            $this->splash('failed',null,'经销商用户不合法',true);
        }
    ...
    {
        $this->member = $this->get_current_member();
        if(!$this->app->model('orders')->getRow('status,pay_status,ship_status',array('order_id'=>$order_id,'member_id'=>$this->member['member_id']))){
            $this->begin(kernel::base_url().'/');
            $this->end(false, '您无权进行此操作');exit;
        }
        ...
        {
            $url = $this->gen_url(array('app' => 'b2c','ctl' => 'site_member', 'act'=>'orderdetail', 'arg0'=>$order_id));
            $this->redirect($url);
        }
        else
        {
    ...
    public function promotion_into_logs($nPage=1)
    {
        $where .= ' where 1 ';
        ...
        $aPage = $this->get_start($nPage,$count_row['row_count']);
        $this->pagination($nPage,$aPage['maxPage'],'balance');
        $this->pagedata['total_money'] = $sum_row['total_money']>0 ? $sum_row['total_money']:0;
        $this->pagedata['list'] = $pils;
