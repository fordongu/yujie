...
class b2c_ctl_admin_goods_editor extends desktop_controller{
   ...
   //新增商品页面ctl
    ...
    function _editor($type_id){
        ...
        $this->pagedata['tichenglist'] = kernel::single('b2c_mdl_ticheng')->getList('tc_id,tc_name');
...
/*}}}*/
    ...
    function toAdd(){        
        ...
        if(!$oGoods->checkPriceWeight($_POST['goods']['product'])){
            $this->end(false,app::get('b2c')->_( '商`品价格或重量格式错误' ));
        }
        ...
        $data = array('tc_id'=>$_POST['goods']['ticheng']['tc_id']);
        $filter = array('goods_id'=>$goods['goods_id']);
        if($oGoods->update($data,$filter)){
            $this->end(true,app::get('b2c')->_('操作成功'),null,$filter);
        }else{
            $this->end(false,app::get('b2c')->_('操作失败'),null,$filter);
        }
}
    ...
    function edit($goods_id){
       ...
       // var_dump($goods['tc_id']);
        file_put_contents("./112text.txt", $goods);
        ksort($goods['images']);
