...
class b2c_ctl_admin_brand extends desktop_controller{
    ...
    function index(){
        $this->finder('b2c_mdl_brand',array(
            'title'=>app::get('b2c')->_('Brand'),
            'actions'=>array(
                array('label'=>app::get('b2c')->_('Add Brand'),'icon'=>'add.gif','href'=>'index.php?app=b2c&ctl=admin_brand&act=create','target'=>'_blank'),
    ...
    function save(){
        ...
        if(empty($_POST['brand_id']) && is_array($brandname)){
             $this->end(false,app::get('b2c')->_('Brand repeat'));
        }
        ...
        if($objBrand->save($data)){
            ...
            #↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑记录管理员操作日志@lujy↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
            $this->end(true,app::get('b2c')->_('Brand successfully saved'));
        }else{
            $this->end(false,app::get('b2c')->_('Failed to save the brand'));
        }
    }

    function edit($brand_id){
        $this->path[] = array('text'=>app::get('b2c')->_('Brand Editor'));
        $objBrand = &$this->app->model('brand');
