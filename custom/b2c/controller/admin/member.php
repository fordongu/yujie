...
class b2c_ctl_admin_member extends desktop_controller{
   ...
   function index(){
        ...
        if($this->has_permission('send_sms')){
            $custom_actions[] =  array('label'=>app::get('b2c')->_('APP消息推送'),'submit'=>'index.php?app=b2c&ctl=admin_member&act=send_huanxin','target'=>'dialog::{title:\''.app::get('b2c')->_('APP消息').'\',width:500,height:350}');
        }

        $actions_base['title'] = app::get('b2c')->_('普通会员列表');
   ...
   function _views(){
    ...
    function add(){
      ...
      if( $member_rpc_object = kernel::service("b2c_member_rpc_sync") ) {
       ...
       //增加父会员邀请会员数
            $db= kernel::database();
            $sql="SELECT invite_mem_fid FROM ".$db->prefix."b2c_members WHERE member_id=".$member_id;
            $result=$db->select($sql);
            if($result[0]['invite_mem_fid']){
                $sql="UPDATE ".$db->prefix."b2c_members SET invite_mem_nums=invite_mem_nums+1 WHERE member_id=".$result[0]['invite_mem_fid'];                
                $db->exec($sql); 
              
                $sql="INSERT INTO ".$db->prefix."microshop_shop_jingxiao(member_id,jx_mem_id,create_time) VALUES(".$result[0]['invite_mem_fid'].",".$member_id.",".time().")";
                $db->exec($sql); 
                
                $sql="SELECT invite_mem_fid FROM ".$db->prefix."b2c_members WHERE member_id=".$result[0]['invite_mem_fid'];
                $result=$db->select($sql);
                if($result[0]['invite_mem_fid']){
                    $sql="INSERT INTO ".$db->prefix."microshop_shop_fenxiao(member_id,fx_mem_id,create_time) VALUES(".$result[0]['invite_mem_fid'].",".$member_id.",".time().")";
                    $db->exec($sql); 
                    ...
                    if($result[0]['invite_mem_fid']){
                        $sql="INSERT INTO ".$db->prefix."microshop_shop_gedai(member_id,gd_mem_id,create_time) VALUES(".$result[0]['invite_mem_fid'].",".$member_id.",".time().")";
                        $db->exec($sql);                    
                    }
            ...
            //同步开设微店，默认关闭状态            
                $sql="INSERT INTO ".$db->prefix."microshop_shop(member_id,addtime,is_open,shop_lvl) VALUES(".$member_id.",".time().",0,'1')";
                $db->exec($sql);                
    ...
    public function send_huanxin(){
        ...
        $this->pagedata['aMember'] = json_encode($aMember);
        $this->page('admin/messenger/huanxin_msg.html');
    }
    ...
    public function huanxin_queue(){
        #$this->begin('index.php?app=b2c&ctl=admin_member&act=index');
        $this->begin();
        //var_dump($_POST['arrMember']);
        $arrMember = json_decode($_POST['arrMember']);
        //var_dump($arrMember);exit();
        $service = kernel::single("b2c_huanxin_registhuanxin");
        $content = trim($_POST['content'],'&nbsp;');
        if(empty($content)){
            $this->end(false,app::get('b2c')->_('推送消息内容不能为空！'));
        }
        $res = $service->send_message('6_MT2sY',$arrMember,$content,'users');
        $data = array(
            'send_mem_id' => 6,
            'send_username' => '6_MT2sY',
            'content' => $content,
            'addtime' => time(),
            'receive_mem_id' => $_POST['arrMember'],            
        );
        if($res['status'] == 200){
            $data['status'] == 'success';
            kernel::single('b2c_mdl_huanxin_sendmsg')->insert($data);
            $this->end(true,app::get('b2c')->_('操作成功！'));
        }
        $data['status'] == 'failure';
        kernel::single('b2c_mdl_huanxin_sendmsg')->insert($data);
