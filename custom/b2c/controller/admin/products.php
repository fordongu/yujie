...
/*-----------------------以上为编辑货品显示数据处理函数-----------------------------*/
 ...
 /*--------------------------以下为保存货品函数--------------------------------------------*/
    ...
    public function save_editor(){
		...
		$yongjin = $_POST['products'];
		foreach($yongjin as $vv=>$kk){
			$commission = $kk['price'] - $kk['cost'];
			kernel::single('b2c_mdl_products')->update(array('commission'=>$commission),array('product_id'=>$kk['product_id']));
		}
        ...
        if(!$goodsFlag){
            ...
            $db->commit();//$returnData
            echo json_encode(array('result'=>'success', 'data'=>json_encode($_POST), 'msg'=>app::get('b2c')->_( '保存成功' )));
            exit;
        }
    }
    ...
    private function _update_goods($goods_id,$productsData,$selectionSpec){
		...
		if(!empty($row['product_id'])){
			$data = array('commission'=>$row['price']['price']['price'] - $row['price']['cost']['price']);
			$abc = $this->app->model('products')->update($data,array('product_id'=>$row['product_id']));
		}
