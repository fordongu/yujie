...
class b2c_ctl_wap_cart extends wap_frontpage{
     ...
     * 
     */
    public function add_wx_address(){
        
        //会员信息
        $arrMember = $this->get_current_member();
        $member_id = $arrMember['member_id'];
        
        $p_province = strip_tags($_POST['province']); //省
        $p_city = strip_tags($_POST['city']); //市
        $p_district = strip_tags($_POST['district']); //区
...
//        $p_district = '沙田镇'; //区
        ...
        //查询与添加地址
        $ectools_regions = app::get('ectools')->model('regions');
        $p_province = str_replace('省',' ', $p_province);
        
        $province_address = $ectools_regions->dump(array('local_name'=>$p_province)); //省
        if(!$province_address){
            $max_p_address = $ectools_regions->getRow('region_id', '', 'region_id DESC ');
            $region_id = $max_p_address['region_id']+1;
            $p_address['region_id'] = $region_id;
            $p_address['local_name'] = $p_province;
            $p_address['package'] = 'mainland';
            $p_address['region_path'] = ','.$region_id.',';
            $p_address['region_grade'] = 1;
            
            $ectools_regions->insert($p_address);
            $province_address = $ectools_regions->dump(array('local_name'=>$p_province)); //省
        }
      
        $city_address = $ectools_regions->dump(array('local_name'=>$p_city)); //市
        if(!$city_address){
            $max_c_address = $ectools_regions->getRow('region_id', '', 'region_id DESC ');
            $region_id = $max_c_address['region_id']+1;
            $c_address['region_id'] = $region_id;
            $c_address['local_name'] = $p_city;
            $c_address['package'] = 'mainland';
            $c_address['p_region_id'] = $province_address['region_id'];
            $c_address['region_path'] = $province_address['region_path'].$region_id.',';
            $c_address['region_grade'] = 2;
            
            $ectools_regions->insert($c_address);
            $city_address = $ectools_regions->dump(array('local_name'=>$p_city)); //市
        }
        
        $district_address = $ectools_regions->dump(array('local_name'=>$p_district)); //区
        if(!$district_address){
            $max_c_address = $ectools_regions->getRow('region_id', '', 'region_id DESC ');
            $region_id = $max_c_address['region_id']+1;
            $d_address['region_id'] = $region_id;
            $d_address['local_name'] = $p_district;
            $d_address['package'] = 'mainland';
            $d_address['p_region_id'] = $city_address['region_id'];
            $d_address['region_path'] = $city_address['region_path'].$region_id.',';
            $d_address['region_grade'] = 3;
            
            $ectools_regions->insert($d_address);
            $district_address = $ectools_regions->dump(array('local_name'=>$p_district)); //区
        }
      
        $b2c_member_addrs = kernel::single('b2c_member_addrs');
        
        /*需要存储的数据*/
        $save_data = array(
            'member_id' => $member_id,
            'area' => 'mainland:'.$province_address['local_name'].'/'.$city_address['local_name'].'/'.$district_address['local_name'].':'.$district_address['region_id'],
            'addr' => $_POST['a_addr'],
            'name' => $_POST['a_name'],
            'zip'  => $_POST['a_zip'],
        ...
        $return_m_data = $member_addrs_model->getList('*',$save_data);
        $return_data = $return_m_data[0];
        
        if(!$return_data){
            $return_data = kernel::single('b2c_member_addrs')->purchase_save_addr($save_data,$member_id,$msg);
        }
        
        if($return_data){
            $addr_id = $return_data['addr_id'];
            $info = json_encode(array("addr_id"=>$addr_id,"area"=>$district_address['region_id']));
            $return = array('status'=>1,'msg'=>'添加成功','info'=>$info);
        }else{
            $return = array('status'=>0,'msg'=>'添加失败','info'=>'');
        }
    ...
    //获取微信收货地址
    public function get_wx_address() {
        ...
        if (strpos($user_agent, 'MicroMessenger') !== FALSE) {
            ...
            //微信参数
            $WeixinBind = app::get('weixin')->model('bind');
            $weinxinInfo = $WeixinBind->dump(array('id'=>1)); 
            $appId = $weinxinInfo['appid'];
            $appSecret = $weinxinInfo['appsecret'];
