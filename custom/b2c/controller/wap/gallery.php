...
class b2c_ctl_wap_gallery extends wap_frontpage{
    ...
    public function jlsfsfssfsf(){
        ...
        $token=file_get_contents('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.$weixinbind['appid'].'&secret='.$weixinbind['appsecret'].'');
        $token=json_decode($token,true);
        if($token['access_token'] && !isset($openid['errcode'])){
            $attention=file_get_contents('https://api.weixin.qq.com/cgi-bin/user/info?access_token='.$token['access_token'].'&openid='.$openid['openid'].'&lang=zh_CN');
            $attention=json_decode($attention,true);
            if(isset($attention['subscribe']) && $attention['subscribe']==0){
                $_SESSION['pay_attention_to']=1;
            }
        }
        $this->redirect($_GET['state']);
    }
    //微信自动登录
    public function adsfssfsfs(){
        if(isset($_GET['code'])){
            
            $weixinbind=app::get('weixin')->model('bind')->dump(array('id'=>1),'*');
            $openid =file_get_contents('https://api.weixin.qq.com/sns/oauth2/access_token?appid='.$weixinbind['appid'].'&secret='.$weixinbind['appsecret'].'&code='.$_GET['code'].'&grant_type=authorization_code');
            $openid=json_decode($openid,true);
            $bindOpenId = app::get('pam')->model('bind_tag')->getRow('member_id',array('open_id'=>$openid['openid']));
            if($bindOpenId){
               $member=app::get('pam')->model('members') ->getRow('*',array('member_id'=>$bindOpenId['member_id']));
               if(!empty($member)){
                    $b2c_members_model = $this->app->model('members');
                    $member_point_model = $this->app->model('member_point');

                    $member_data = $b2c_members_model->getList( 'member_lv_id,experience,point', array('member_id'=>$member['member_id']) );
                    if(!empty($member_data)){
                        //设置会员登录session member_id
                        $this->userObject = kernel::single('b2c_user_object');
                        $this->userObject->set_member_session($member['member_id']);

                        $member_data['order_num'] = $this->app->model('orders')->count( array('member_id'=>$member['member_id']) );
                        ...
                        {
                            $member_data['member_lv_id'] = $member_point_model->member_lv_chk($member['member_id'],$member_data['member_lv_id'],$member_data['point']);
                        }
                        $this->bind_member($member['member_id']);
            ...
            if($token['access_token'] && !isset($openid['errcode'])){
                ...
                if(isset($attention['subscribe']) && $attention['subscribe']==0){
                    ...
                    unset($_SESSION['pay_attention_to']);
                }
            }
            
        }
        if(!isset($_SESSION['account']['member'])){
            $_SESSION['account']['member']=0;
        }
        $this->redirect($_GET['state']);
    ...
    public function aaa(){
        $a=kernel::single('mobileapi_rpc_member');
        $b=$a->setting();
        var_dump($b);exit;
        // $url=file_get_contents('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wx6c64a8aa97944124&secret=8a0643bce99718412e7758ed26fd3e8f');
        // $model=json_decode($url,true);
        // $this->pagedata['token']=$model['access_token'];
        // $this->page('wap/gallery/a.html');
    }
