...
if(isiOS){
    url = url+'';
                ...
                alert(123);
document.getElementById("xiazai").href= url
</script>
Eof;
    }
        ...
        if( $_SESSION['weixin_u_openid'] && !$bindOpenId && !$bindMember ){
            ...
            if( $_SESSION['weixin_u_openid'] && $bindMember && $_SESSION['is_bind_weixin'] ){
                ...
                if (strpos($user_agent, 'MicroMessenger') === false) {
                    ...
                    $url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$weixinbind['appid'].'&redirect_uri='.urlencode(WWW_URL.'/index.php/wap/member-callback.html').'&response_type=code&scope=snsapi_base#wechat_redirect'; 
   ...
   /**
    ...
    public function create(){
        ...
        if($_SESSION['inivte_id'] > 0){
            ...
            if($res[0]['member_id'] > 0){
                ...
                $this->splash('failed',null,'您填写的邀请码不对','','',true);exit();
            }
        }else{
            $invite_mem_fid = kernel::single('b2c_ticheng_rand')->invitecode();
        }

        $_POST['vcode'] = $_POST['vcode'];
    ...
    public function nickname(){
        echo kernel::single('b2c_ticheng_rand')->nickname();
    }
    /**
     * 随机获取邀请码
     */
    public function invitecode(){
        echo kernel::single('b2c_ticheng_rand')->invitecode();
    }
}