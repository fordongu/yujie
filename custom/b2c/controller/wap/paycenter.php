...
class b2c_ctl_wap_paycenter extends wap_frontpage {
    ...
    public function index($order_id, $newOrder = false) {
        ...
        //微信支付参数
        $this->pagedata['jsApiParameters'] = kernel::single('weixin_payment_pay4_JsApipub')->getParameters();
        ...
        if ($pay_object) {
            ...
            if (strpos($user_agent, 'MicroMessenger')) {
                ...
                if ($matches[2] > 5.1) {
                    if (!isset($_GET['code'])) {
                        $_SESSION['wxpay'] = null;
                        unset($_SESSION['wxpay']);
                        $_SESSION['postdata'] = $_POST;
                        $jsApi = kernel::single('weixin_payment_pay4_JsApipub');
                        $weixin_url = $jsApi->createOauthUrlForCode('http://www.zfjk800.com'. $_SERVER['REQUEST_URI']);
                        header('location:' . $weixin_url);
                        exit();
                    } else {
                        $_SESSION['wxpay']['code'] = $_GET['code'];
                        $_POST = $_SESSION['postdata'];
                        $_SESSION['postdata'] = null;
                    }
                } else {
                    logger::info('微信浏览器的版本不够 : ' . var_export($user_agent, 1));
                }
            }
    ...
    public function result_pay($data_id, $type = false) {
        ...
        $arrOrders = app::get('b2c')->model('orders')->getList('order_id,payed,final_amount,cur_rate,currency,total_amount,ship_area,ship_addr,ship_zip,ship_time,ship_mobile,ship_name,ship_tel,pay_status', array('order_id' => $order_id));
    ...
    public function weixinPay() {
        ...
        require_once "./weixin/unit/WxPay.JsApiPay.php";
        require_once './weixin/unit/log.php';
