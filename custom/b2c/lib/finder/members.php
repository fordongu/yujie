...
class b2c_finder_members {
    ...
    var $pagelimit = 10;
    var $column_see_special;
    var $column_see_products;
    ...
    public function __construct($app) {
        ...
        $this->detail_zhijie = app::get('b2c')->_('我的直接下线');
        //$this->detail_jianjie = app::get('b2c')->_('我的简接下线');
        ...
        $this->column_see_special = app::get('b2c')->_('查看微店专辑');
        $this->column_see_products = app::get('b2c')->_('查看微店商品');
    }
    ...
    function detail_edit($member_id) {
        ...
        if ($_POST) {
            //var_dump($_POST);exit();
            $filed = 'member_lv_id,invite_mem_fid,member_id';
            $member = $member_model->getList($filed, array('member_id' => $member_id));
            $member = $member[0];

            //等级修修改 ，判断
            if ($_POST['member_lv']['member_group_id'] != $member['member_lv_id']) {//有修改
                $n_member = $member_model->getList($filed, array('invite_mem_fid' => $member_id));
                $n_member = $n_member[0];
                $mem = kernel::single('b2c_mdl_member_lv')->getList('class,role',array('member_lv_id' => $_POST['member_lv']['member_group_id']));
                $mem = $mem[0];
                switch ($mem['class']) {
                    case 'one': $mem['num'] = 1;
                        break;
                ...
                if ($member['invite_mem_fid'] > 0) {//
                    $f_mem = $member_model->get_member_type($member['invite_mem_fid']);
                    if ($mem['num'] >= $f_mem['num']) {
                        header('Content-Type:text/jcmd; charset=utf-8');
                        echo '{error:"修改等级不能超过上级的等级！",_:null}';
                        exit();
                    }
                }
                if ($n_member['member_id'] > 0) {
                    $n_mem = $member_model->get_member_type($n_member['member_id']);
                    //var_dump($n_mem);exit();
                    if ($mem['num'] <= $n_mem['num'] && $n_mem['role'] == '1') {
                        header('Content-Type:text/jcmd; charset=utf-8');
                        echo '{error:"修改等级不能低于下级等级！",_:null}';
                        exit();
                    }
                }
                //修改为 三星代理 （砖石代理）
                if($men['class'] == 'thr' && $mem['role'] = 'dai'){
            ...
            if ($member_model->save($saveData['b2c_members'])) {
               ...
               @kernel::single('b2c_huanxin_registhuanxin')->edit_nickname($member_id,$_POST['nickname']);
        ...
        //var_dump($membersData['members']['nickname']);
    ...
    function detail_advance($member_id = null) {
        ...
        //$data = $mem_adv->getList('*',array('member_id'=>$member_id),$this->pagelimit*($nPage-1),$this->pagelimit,'mtime desc');
        //var_dump($data);
    ...
    //我的直接下线
    function detail_zhijie($member_id = null) {
        if (!$member_id)
            return null;
//      $nPage = $_GET['detail_zhijie'] ? $_GET['detail_zhijie'] : 1;
        $app = app::get('b2c');
        $member = $app->model('members');
        $member_lv = $app->model('member_lv');
        $filet = 'member_id,name,member_lv_id,mobile,regtime,order_num,nickname';
        $zhijie = $member->getList($filet, array('invite_mem_fid' => $member_id), $this->pagelimit * ($nPage - 1), $this->pagelimit);
        foreach ($zhijie as $k => $v) {
            $res = $member_lv->dump(array('member_lv_id' => $v['member_lv_id']), 'name,role');
            $zhijie[$k]['member_lv'] = $res;
        }
        $lv_name = kernel::single('b2c_mdl_member_lv')->getList('name', array('member_lv_id' => $zhijie[0]['member_lv_id']));
        $zhijie[0]['lv_name'] = $lv_name[0]['name'];
        $render = $app->render();
        $render->pagedata['zhijie'] = $zhijie;
        if ($_GET['page'])
            unset($_GET['page']);
        $_GET['page'] = 'detail_zhijie';
        $this->controller->pagination($nPage, $count, $_GET);
        return $render->fetch('admin/member/zhijie.html');
    }

    //我的间接下线
    // function detail_jianjie($member_id=null){
    //}
    ...
    var $column_see_special_order = 16;

    public function column_see_special($row) {
        if (!$this->pam_member_info[$row['member_id']]) {
        ...
        return $pam_member_info['members']['member_type'] == 2 ? '<a href="index.php?app=microshop&ctl=admin_special&act=index&filter[member_id]=' . $row['member_id'] . '">' . $this->column_see_special . '</a>' : '不是微店用户';
    }

    var $column_see_products_order = 17;

    public function column_see_products($row) {
        if (!$this->pam_member_info[$row['member_id']]) {
            $pam_member_info = $this->userObject->get_members_data(array('account' => 'login_account', 'members' => '*'), $row['member_id']);
        } else {
            $pam_member_info = $this->pam_member_info[$row['member_id']];
        }
        return $pam_member_info['members']['member_type'] == 2 ? '<a href="index.php?app=microshop&ctl=admin_products&act=index&filter[member_id]=' . $row['member_id'] . '">' . $this->column_see_products . '</a>' : '不是微店用户';
    }
