...
class b2c_message_comment{
    ...
    function __construct(&$app){
     ...
     function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderby=null){
        ...
        $memberLvIds = $objMember->getList('member_id,member_lv_id,avatar',array('member_id'=>$memberIds));
        foreach($memberLvIds as $row){
            ...
            $row['member_avatar'] = $row['avatar'] ? base_storager::image_path($row['avatar']) : kernel::base_url('full').'/public/app/b2c/statics/sprites/default-avatar.png';
        ...
        foreach($aData as $key => $val){
            ...
            $val['member_avatar'] = $tmpMemberLv[$val['author_id']]['member_avatar'];
