...
class b2c_order_create extends b2c_api_rpc_request
{
    ...
    private function _chgdata(&$sdf, &$order_data, $member_indent='', &$msg='',$aCart=array())
    {
        ...
        //$order_data['total_amount'] = $sdf['total_amount'];
        ...
        $order_data['cur_amount'] = $order_data['total_amount'] - $order_data['num'];
                ...
                if (is_array($arrObjInfo) && $arrObjInfo)
                {
                    ...
                    if ($obj_type == "goods")
                    {
                        ...
                        foreach ($arrObjInfo as $arrObjItems)
                        {
                            ...
                            //print_r($arrObjItems['params']['extends_params']['buy_code']); exit;  
                        	$buy_code = isset($arrObjItems['params']['extends_params']['buy_code'])?$arrObjItems['params']['extends_params']['buy_code']:'';
    ...
    protected function request($sdf, $method, $callback=array(), $title, $time_out=1, $rpc_id=null)
    {
        ...
        $arr_data['is_cod'] = $arr_payment['is_cod']; //支付方式支持是否“货到付款”
