            ...
            foreach ($order_items as $key=>$dinfo)
            {
                if ($dinfo['item_type'] == 'product' || $dinfo['item_type'] == 'adjunct')
                {
                    ...
                    {//弹窗并且输入了发货数量>=1
                        ...
                        $dinfo['agency_no'] = $sdf['agency_no'][$dinfo['item_id']];
                    }
                    else
                    {
                ...
                foreach ($order_items as $dinfo)
                {
                    if($dinfo['send'])
                    {
                        $item = array(
                                ...
                                'product_bn' => $dinfo['bn'],
                                'product_name' => (isset($dinfo['addon']['adjname']) && $dinfo['addon']['adjname']) ? $dinfo['name'].$dinfo['addon']['adjname'] : $dinfo['name'],
                                'number' => $dinfo['send'],
                        		'agency_no' => $dinfo['agency_no'],
                            );
