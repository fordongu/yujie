...
//end function
    ...
    public function pre_signup_process($data) {
        ...
        $data['mobile'] = $accountData['login_account'];
        //---end
        $return = array(
...
//end function
    ...
    public function save_members($saveData, &$msg) {
        ...
        if ($member_model->save($saveData['b2c_members'])) {
            ...
            //环信 会员数据同步
            kernel::single('b2c_huanxin_registhuanxin')->saveuser($member_id,$saveData['b2c_members']['nickname']);
            ...
            $data = array('mobile'=>$saveData['pam_account']['login_account']);
            $member_model->update($data,$filter);
            
            $data = array(
                'shop_id' => $member_id,
                'shop_name' => $member_id . '号微店铺',
                'member_id' => $member_id,
                'addtime' => time(),
                'is_open' => 'on',
                'shop_lvl' => 1,
            ...
            $res = kernel::single('microshop_mdl_shop')->insert($data);
            /*--------- 会员数据同步处理 ----------*/
            $db->commit();
        } else {
            $msg = '保存失败，请联系客服!';
    ...
    public function update_invite($invite_mem_fid, $member_id) {
        $Members = kernel::single('b2c_mdl_members');
        $res = $Members->getList('super_mem_id,daili_path,member_lv_id', array('member_id' => $invite_mem_fid));
        if (!empty($res[0]['daili_path'])) {
            $super_mem_id = $res[0]['super_mem_id'];
            $count = count(explode('-', $res[0]['daili_path']));
            switch ($count) {
                case 1:
                    logger::debug('daili_path is error ,member_id:' . $member_id);
                    $daili_path = $invite_mem_fid . '-' . $member_id;
                    break;
                case 2:
                    $daili_path = $res[0]['daili_path'] . '-' . $member_id;
                    break;
                case 3:
                    $daili_path = $res[0]['daili_path'];
                    break;
            }
        } else {//顶级代理
            $super_mem_id = $invite_mem_fid;
            $daili_path = $invite_mem_fid . '-' . $member_id;
        }


        $data = array(
            'invite_mem_fid' => $invite_mem_fid, //上级id
            'super_mem_id' => $super_mem_id, //顶级代理ID
            'daili_path' => $daili_path, //代理path路径
        );
        $filter = array('member_id' => $member_id); //
        if($Members->update($data, $filter)){
            //加入 伙伴儿群中
            kernel::single('b2c_huanxin_group')->add_group_username($invite_mem_fid,$member_id);            
        }
        return false;
    }
