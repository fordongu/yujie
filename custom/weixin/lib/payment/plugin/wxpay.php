...
final class weixin_payment_plugin_wxpay extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    function callback(&$in) {
        if ($_POST['weixin_postdata']['result_code'] == 'SUCCESS' && $_POST['weixin_postdata']['return_code'] == 'SUCCESS') {
            $_POST = $_POST['weixin_postdata'];
            $_POST['weixin_postdata'] = null;
            unset($_POST['weixin_postdata']);
            $_POST['trade_state'] = '0';
        }
        ...
        $paySignKey = trim($this->getConf('paySignKey', __CLASS__)); // PaySignKey 对应亍支付场景中的 appKey 值
        $partnerId = trim($this->getConf('partnerId', __CLASS__)); // 财付通商户身份的标识
        $partnerKey = trim($this->getConf('partnerKey', __CLASS__)); // 财付通商户权限密钥 Ke
...
//↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑公共函数部分↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    ...
    private function paydata_update($data) {
        if (!empty($data)) {
            $Orders = kernel::single('b2c_mdl_orders');
            $Memadv = kernel::single('b2c_mdl_member_advance');
            $Payments = kernel::single('ectools_mdl_payments');
                
            $payment_id = $data['attach'];
            $order_id = $data['out_trade_no'];
            $filter_pay = array('payment_id' => $payment_id);
            $payments = $Payments->getList('member_id,status,pay_type,money', $filter_pay);
            $member_id = $payments[0]['member_id'];
            logger::info('lkjalkdjlkfajlksdjlfkajlsdf'.  var_export($data,1));
            if ($data['trade_state'] == '0' || $data['result_code'] == 'SUCCESS') {//支付成功
                //流水账号记录               
                if ($payments[0]['money'] * 100 == $data['total_fee']) {//支付的金额和与支付的金额
                    if ($member_id > 0 && $payments[0]['status'] == 'ready' && $payments[0]['pay_type'] == 'online') {
                        $data1 = array(
                            'status' => 'succ',
                            't_payed' => time(),
                            'payment_bn' => $data['transaction_id'],
                            'memo' => $order_id,
                            'trade_no' => '',
                            'thirdparty_account' => '',
                        );
                        $Payments->update($data1, $filter_pay);
            
                        //订单状态修改
                        $filter = array('order_id' => $order_id);
                        $data1 = array('pay_status' => 1, 'last_modified' => time());
                        $res = $Orders->update($data1, $filter);
                        //$order = $Orders->getList('status,pay_status,ship_status', $filter);
                        if (!$res) {
                            logger::error('***没有相应的带支付的订单,订单ID:' . $order_id);
                        }
                        //会与资金交易记录
                        $shop_adv = $Memadv->get_shop_advance();
                        $mem_adv = $Memadv->get($member_id);
                        $data1 = array(
                            'member_id' => $member_id,
                            ...
                            'explode_money' => $data['total_fee'] / 100,
                            'shop_advance' => $shop_adv,
                        );
                        $memberadv = $Memadv->insert($data1);
                    } else {
                        logger::error('支付单表已经没有预处理记录,订单ID:' . $order_id);
                    }
                } else {//支付金额异常
                    $data1 = array(
                        'status' => 'failed',
                        't_payed' => time(),
                        'payment_bn' => $data['transaction_id'],
                        'memo' => '支付异常,微信支付金额错误:' . json_encode($data),
                        'trade_no' => $order_id,
                        'return_url' => $data['notify_id'],
                        'thirdparty_account' => '',
                    );
                    $Payments->update($data1, $filter_pay);
                    logger::error('支付金额和预支付订单的金额不对,订单ID:' . $order_id);
                }
            } else {//支付失败或者异常
                $data1 = array(
                    'status' => 'failed',
                    't_payed' => time(),
                    'payment_bn' => $data['transaction_id'],
                    'memo' => '微信支付失败:' . json_encode($data),
                    'trade_no' => $order_id,
                    'return_url' => $data['notify_id'],
                    'thirdparty_account' => '',
                );
                $aaaa = $Payments->update($data1, $filter_pay);
                logger::error('微信支付不成功,订单ID:' . $order_id);
            }
        }
    }
